...
const jsCode = `(async function() {
  try {
    ...
    // Find any element containing stop-like SVGs or paths
    const allSvgs = Array.from(document.querySelectorAll('svg')).map(s => s.outerHTML);

    return { classBefore, htmlBefore, classStreaming, htmlStreaming, allSvgs: allSvgs.slice(0, 10) };
  } catch (err) {
